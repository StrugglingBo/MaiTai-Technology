...
namespace DTcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "user_verify_email": //发送注册验证邮件
                    ...
                    break;
                case "user_repwdbymobile"://通过验证码直接获取新面貌
                    user_repwdbymobile(context);
                    break;
                ...
                case "order_save": //保存购物订单
                    order_save(context);
                    break;
                case "shoporder_save": //保存购物订单
                    shoporder_save(context);
                    break;
        ...
        #region 通过手机验证码直接获取新密码
        private void user_repwdbymobile(HttpContext context)
        {
            ...
            string password = Utils.Number(6).ToString();

            Model.users userModel = new BLL.users().GetModel(mobile);
            userModel.salt = Utils.GetCheckCode(6);
            userModel.password = DESEncrypt.Encrypt(password, userModel.salt);

            Model.sms_template smsModel = new BLL.sms_template().GetModel("user_repassword"); //取得短信内容
            if (smsModel != null && new BLL.users().Update(userModel))
            {
                //替换标签
                string msgContent = smsModel.content;
                msgContent = msgContent.Replace("{webname}", siteConfig.webname);
                ...
                msgContent = msgContent.Replace("{code}", password);
                //发送短信
                string tipMsg = string.Empty;
                new BLL.sms_message().Send(userModel.mobile, msgContent, 2, out tipMsg);
            }
            context.Response.Write("{\"status\":0, \"msg\":\"新密码已发送至" + mobile + "手机上.请注意查收!\"}");
            return;
        }
        #endregion
        ...
        private void user_register(HttpContext context)
        {
            string site = DTRequest.GetQueryString("site").Trim(); //当前站点
            string code = DTRequest.GetFormString("txtCode").Trim();
            string tuijianren = Utils.ToHtml(DTRequest.GetFormString("txtUserName").Trim());
            string password = DTRequest.GetFormString("txtPassword").Trim();
            string mobile = Utils.ToHtml(DTRequest.GetFormString("txtMobile").Trim());
            string userip = DTRequest.GetIP();
            ...
            if (string.IsNullOrEmpty(tuijianren))
            {
                    ...
                    //tuijianren = "13606532312";
                
                    tuijianren = "18815168888";
                
                //context.Response.Write("{\"status\":0, \"msg\":\"错误：推荐人不能为空！\"}");
                //return;
                Model.users tjuserModel = new BLL.users().GetModel(tuijianren);
                if (tjuserModel == null)
                {
                    context.Response.Write("{\"status\":0, \"msg\":\"错误：不存在该推荐人！\"}");
                    return;
                }
            ...
            if (tuijianren != "")
            {
                Model.users tjuser = new BLL.users().GetModel(tuijianren);
                if (tjuser != null)
                {
                    if (tjuser.group_id != 3)
                    {
                        model.nick_name = tuijianren;
                    }
                    model.area = tjuser.area;
                }
            }
            //设置用户状态
            ...
            else
            {
                ...
                context.Response.Write("{\"status\":1, \"msg\":\"注册成功，欢迎成为本站会员！\", \"url\":\""
                    + new Web.UI.BasePage().getlink(site, new Web.UI.BasePage().linkurl("usercenter")) + "\"}");
            }
            return;
        }
        #endregion
        ...
        private void user_password_edit(HttpContext context)
        {
            ...
            string password = DTRequest.GetFormString("txtPassword");
            string password1 = DTRequest.GetFormString("txtPassword1");
            //检查输入密码是否正确
            if (!password.Trim().Equals(password1.Trim()))
            {
                context.Response.Write("{\"status\":0, \"msg\":\"您输入的两次新密码不一致！\"}");
                return;
            }
            //检查输入的旧密码
        ...
        #region 保存在线订单================================
        private void shoporder_save(HttpContext context)
        {
            //获取传参信息===================================
            int pid = int.Parse(context.Request["pid"].ToString());
            string sitepath = context.Request["site"].ToString(); //站点目录
            decimal amount = decimal.Parse(context.Request["amount"].ToString());//需要支付金额

            //检查传参信息===================================
            if (pid <= 0)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，无法获取店铺信息！\"}");
                return;
            }
            //检查站点目录
            if (string.IsNullOrEmpty(sitepath))
            ...
            //获取商品信息==================================
            Model.article artproinfo = new BLL.article().GetModel(pid);
            if (artproinfo != null)
            {
                int tpoint = int.Parse(((decimal.Parse(artproinfo.fields["prevalue"].ToString()) * amount) / 100).ToString("F0"));
                decimal tprice = amount - tpoint;
                //保存订单=======================================
                ...
                model.express_id = 1;
                model.accept_name = "在线付款";
                model.area = ""; //省市区以逗号相隔
                model.address = artproinfo.fields["sub_title"].ToString();
                model.telphone = "";
                model.mobile = userModel.telphone;
                if (tpoint > 0)
                {
                    model.message = "恭喜您,本次购买可用积分抵扣:" + tpoint + "元";
                }
                else
                {
                    model.message = "";
                ...
                if (model.point > 0)
                {
                    if (model.point <= userModel.point)
                    {
                        ...
                        {
                            new BLL.user_point_log().Add(model.user_id, model.user_name, model.point * -1, "本地生活，订单号：" + model.order_no, false);
                        }
                    }
                    else
                    {
                        context.Response.Write("{\"status\":0, \"msg\":\"下单失败,您的积分不足！\"}");
                        return;
                    }
                }
                //提交成功，返回URL
            ...
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，您购买的店铺不存在或已下架！\"}");
                return;
            }
        }
        ...
        private void order_save(HttpContext context)
        {
            //获取传参信息===================================
            int pid = int.Parse(context.Request["pid"].ToString());
            int bnum = int.Parse(context.Request["bnum"].ToString());
            string sitepath = context.Request["site"].ToString(); //站点目录
            string accept_name = Utils.ToHtml(context.Request["accept_name"].ToString());
            string province = Utils.ToHtml(context.Request["province"].ToString());
            string city = Utils.ToHtml(context.Request["city"].ToString());
            string area = Utils.ToHtml(context.Request["area"].ToString());
            string address = Utils.ToHtml(context.Request["address"].ToString());
            string mobile = Utils.ToHtml(context.Request["mobile"].ToString());
            string goodnum = context.Request["goodno"].ToString();
            string post_card = context.Request["post_card"].ToString();
            string carduname = context.Request["carduname"].ToString();
            string postid = context.Request["postid"].ToString();
            ...
            //{
            //    //检查卡号持有人姓名
            //    if (string.IsNullOrEmpty(carduname))
            //    {
            //        context.Response.Write("{\"status\":0, \"msg\":\"对不起，您必须填写卡号持有人姓名！\"}");
            //        return;
            //    }
            //}
            //如果开启匿名购物则不检查会员是否登录
            ...
            if (userModel != null)
            {
                ...
                if (postid != "0")
                {
                    addrModel = new BLL.user_addr_book().GetModel(int.Parse(postid));
                }
                if (addrModel == null)
                {
                    Model.user_addr_book bModel = new Model.user_addr_book();
                    bModel.user_id = userModel.id;
                    bModel.user_name = userModel.user_name;
                    bModel.accept_name = accept_name;
                    bModel.area = province + "," + city + "," + area;
                    bModel.address = address;
                    bModel.mobile = mobile;
                    bModel.add_time = DateTime.Now;
                    bModel.post_code = post_card;
                    bModel.is_default = 1;
                    new BLL.user_addr_book().Add(bModel);
                }
            }
            ...
            if (artproinfo != null)
            {
                decimal tprice = decimal.Parse(artproinfo.fields["sell_price"].ToString()) * bnum;
                int tpoint = int.Parse(artproinfo.fields["point"].ToString()) * bnum;
                ...
                if (tpoint <= 0)
                {
                    if (tprice >= 3060)
                    {
                        zkou = decimal.Parse(((rd.Next(0, 2)) + (rd.NextDouble())).ToString("F2"));
                        tprice = tprice - zkou;
                    }
                }
                //保存订单=======================================
                ...
                if (zkou != 0)
                {
                    model.message = "恭喜您,本次购买自动免付:" + zkou + "元";
                }
                else
                {
                    model.message = "";
                ...
                model.email = carduname;
                model.post_code = post_card;
                model.is_invoice = 0;
                model.payable_amount = tprice;
                model.real_amount = tprice;
                //购物积分,正数为扣除,负数为增加
                ...
                model.add_time = DateTime.Now;
                string stxt = "";
                decimal gp = decimal.Parse(artproinfo.fields["sell_price"].ToString());
                if (goodnum != "")
                {
                    DataSet ds = new BLL.article_goods().GetList("goods_no='" + goodnum + "'");
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        stxt = ds.Tables[0].Rows[0]["spec_text"].ToString();
                        gp = decimal.Parse(ds.Tables[0].Rows[0]["sell_price"].ToString());
                    }
                }
                //商品详细列表
                List<Model.order_goods> gls = new List<Model.order_goods>();
                gls.Add(new Model.order_goods
                {
                    article_id = artproinfo.id,
                    goods_id = artproinfo.id,
                    goods_no = goodnum == "" ? artproinfo.fields["goods_no"] : goodnum,
                    goods_title = artproinfo.title,
                    img_url = artproinfo.img_url,
                    spec_text = stxt,
                    goods_price = gp,
                ...
                if (carduname != "")
                {
                    ...
                    model.remark = carduname + "," + post_card;
                }
                int result = new BLL.orders().Add(model);
                if (result < 1)
                {
                ...
                if (model.point > 0)
                {
                    if (model.point <= userModel.point)
                    {
                        ...
                        if (new BLL.users().Update(userModel))
                        {
                            new BLL.user_point_log().Add(model.user_id, model.user_name, model.point * -1, "积分换购，订单号：" + model.order_no, false);
                        }
                    }
                    else
                    {
            ...
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，您购买的产品不存在或已下架！\"}");
                return;
            }
        }
