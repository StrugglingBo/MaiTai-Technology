...
namespace DTcms.DAL
{
    ...
    public partial class users
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select '`'+du.user_name as 会员账号,ui.truename as 姓名,'`'+ui.identyno as 身份证号,(case du.group_id when 1 then '普通会员' when 2 then 'VIP' when 3 then '代理商' end ) as 会员类型,'`'+du.mobile as 手机号码,nick_name as 推荐人,area as 所在区域,avatar as 我的业绩指数,amount as 今日积分,point as 可用积分,exp as 已提现积分,'`'+ui.cardno as 银行卡号,ui.bankname as 所属银行,ui.alipaynickname as 开户行,birthday as VIP到期时间,reg_time as 注册时间,(select MAX(login_time) from dt_user_login_log where user_id=du.id) as 最近登录时间,(select MAX(login_ip) from dt_user_login_log where user_id=du.id) as 最近登录IP,reg_ip as 注册IP,(case status when 0 then '正常' when 1 then '待验证' when 2 then '待审核' when 3 then '已禁用' end ) as 用户状态 from dt_users du left join dt_user_info ui on du.id=ui.userid");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// <returns></returns>
        public string gettotaluser(string area)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) as t from " + databaseprefix + "users");
            strSql.Append(" where area='" + area + "'");
        ...
        /// <param name="area"></param>
        /// <returns></returns>
        public string gettodayuser(string area)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) as t from " + databaseprefix + "users");
            strSql.Append(" where area='" + area + "' and convert(char(10),reg_time,120)=convert(char(10),getdate(),120)");
            return DbHelperSQL.GetSingle(strSql.ToString()).ToString();
        }
        public string gettotalguser(string username)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) as t from " + databaseprefix + "users");
            strSql.Append(" where nick_name='" + username + "' and group_id=2");
            return DbHelperSQL.GetSingle(strSql.ToString()).ToString();
        }
        public string gettotalpuser(string username)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) as t from " + databaseprefix + "users");
            strSql.Append(" where nick_name in (select user_name from dt_users where nick_name='" + username + "' and group_id=2) and group_id=2");
            return DbHelperSQL.GetSingle(strSql.ToString()).ToString();
        }
        /// <summary>
        /// 修改一列数据
        ...
        public double getavgpre() {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select (select SUM(convert(decimal,avatar)) from dt_users where ISNULL(avatar,'')>'' )/ (select COUNT(1) from dt_users where ISNULL(avatar,'')>'')");
            return double.Parse(DbHelperSQL.GetSingle(strSql.ToString()).ToString());
        }
        /// <summary>
        /// 得到一个对象实体
        /// </summary>
