...
function initGoodsSpec(sendUrl) {
    //检查是否有规格
    if (($("#goodsSpecBox li").length - 2) == 0) {
        $("#buyButton button").prop("disabled", false).removeClass("over");
    }
    //遍历规格属性
    $("#goodsSpecBox li div.sub-option a").each(function () {
        $(this).click(function () {
            if (!$(this).hasClass("active")) {
                ...
                //获取商品价格
                if ($("#goodsSpecBox li div.sub-option a.active").length == ($("#goodsSpecBox li").length - 2)) {
                    var specIds = '';
                    $("#goodsSpecBox li div.sub-option a.active").each(function (i) {
                        if (i == 0) {
                            specIds = ",";
                        }
                    ...
                    $.ajax({
                        ...
                        success: function (data, textStatus) {
                            if (data.status == 1) {
                                $("#commodityGoodsNo").val(data.goods_no);
                                $("#showmoeny").text('¥' + data.sell_price);
                                $("#sprice").val(data.sell_price);
                                $("#commodityStockNum").text("库存:" + data.stock_quantity);
