...
namespace DTcms.DAL
{
	...
	public partial class user_recharge
	{
		...
		public int Add(Model.user_recharge model)
		{
			...
			strSql.Append("user_id,user_name,recharge_no,payment_id,amount,status,add_time,complete_time,mypre)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@mypre", SqlDbType.Decimal,5)};
            ...
            parameters[8].Value = model.mypre;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select USER_NAME as 申请人,amount as 提现积分,trueamount as 实际金额,mypre as 业绩指数,(case status when 0 then '申请中' when 1 then '已打款' when 2 then '审核不通过' end) as 当前状态,('`'+cardno) as 银行卡号,truename as 真实姓名,bankname as 银行名称,alipaynickname as 开户行,(case CHARINDEX('建设',bankname) when 0 then '02' else '01' end) as 是否本行,add_time as 申请时间,complete_time as 打款时间 ");
            strSql.Append(" from v_recharge_detail ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM V_recharge_detail");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
