...
namespace DTcms.DAL
{
    ...
    public partial class orders
    {
        ...
        public DataSet GetdetailList(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            strSql.Append("order_no as 订单号,'`'+USER_NAME as 用户名,'`'+trade_no as 核账单号,(select title from dt_payment where id =view_order_detail.payment_id) as 支付方式 ,(case payment_status when 1 then '待支付' when 2 then '已支付' end) as 支付状态 ,payment_time as 支付时间, (select title from dt_express where id=view_order_detail.express_id) as 快递方式,express_no as 快递单号,(case express_status when 1 then '未发货' when 2 then '已发货' end) as 支付状态,express_time as 发货时间,accept_name as 收件人,'`'+telphone as 收件人号码,'`'+post_code as 油卡编号,email as 油卡持有人姓名,(area+address) as 寄送地址,remark as 订单备注,(case is_invoice when 0 then '无需发票' when 1 then '需要发票' end) as 是否需要发票,invoice_title as 发票抬头,order_amount as 订单总额,(case status when 1 then '支付处理' when 2 then '发货处理' when 3 then '交易完成' when 4 then '订单取消' when 5 then '订单作废' end) as 是否需要发票,add_time as 下单时间,goods_title as 产品名称,spec_text as 产品属性,quantity as 产品数量,'`'+author as 供应商账户,userarea as 代理商区域 from View_order_detail");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public DataSet getlistbysql()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select order_no as 订单号,'`'+trade_no as 核帐单号,user_name as 用户名,payment_time as 支付时间,accept_name as 收货人,remark as 备注,add_time as 下单时间,confirm_time as 确认时间 from dt_orders where order_no like 'B%' and payment_status=2 and status=2 and id in (select order_id from dt_order_goods where ISNULL(goods_no,'')='' and goods_price>=3000)");
            return DbHelperSQL.Query(strSql.ToString());
        }
