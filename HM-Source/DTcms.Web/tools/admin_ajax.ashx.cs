        ...
        private void edit_order_status(HttpContext context)
        {
            ...
            switch (edit_type.ToLower())
            {
                ...
                case "order_payment": //确认付款
                    ...
                    model.status = 2;
                    //判断是否为T开头
                    if (model.order_no.StartsWith("T")) {
                        model.status = 3;
                ...
                case "order_complete": //完成订单=========================================
                    ...
                    Model.users umodel2 = new BLL.users().GetModel(model.user_id);
                    if (umodel2.group_id == 1) {
                        if (model.order_no.StartsWith("B") && model.order_amount>=2997 && umodel2.msn!="1") {
                            umodel2.msn = "1";
                            if (umodel2.qq == "1") {
                                umodel2.group_id = 2;
                            }
                            new BLL.users().Update(umodel2);
                        }
                    }
                    if (model.order_no.StartsWith("B")) {
                        //给代理商增加2%个积分
                        DataSet ds = new BLL.users().GetList(1, "group_id=3 and area='" + umodel2.area + "'", "");
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            int pid = int.Parse(ds.Tables[0].Rows[0]["id"].ToString());
                            Model.users puserModel = new BLL.users().GetModel(pid);
                            int addpoint = int.Parse((double.Parse(model.order_amount.ToString())*0.02).ToString());
                            ...
                            if (new BLL.users().Update(puserModel))
                            {
                                ...
                                pupmodel.remark = "区域中会员[" + umodel2.user_name + "]购买商品获得";
                ...
                case "order_cancel": //取消订单==========================================
                    ...
                    if (!bll.Update(model))
                    {
                        ...
                        if (model.point > 0)
                        {
                            ...
                            {
                                new BLL.user_point_log().Add(model.user_id, model.user_name, model.point, "积分换购取消，订单号：" + model.order_no, false);
                            }
                        }
